...
const EventImpact = () => {
    ...
    useEffect(() => {
        const fetchEventData = async () => {
            try {
                const response = await axios.get('http://localhost:5001/api/event-impact');
        ...
        const fetchPriceTrends = async () => {
            try {
                const response = await axios.get('http://localhost:5001/api/price-trends');
                ...
                <tbody>
                    {eventData.map((event, index) => (
                        <tr key={index}>
                            ...
                            <td>{event["P-Value"] !== null 
          ? (event["P-Value"] < 0.0001 ? event["P-Value"].toExponential(4) : event["P-Value"].toFixed(4))
          : 'N/A'}</td>
                        </tr>
                    ))}
                </tbody>
